...
// ============================================================================
// INSTRUCTIONS FOR YOUR LOCAL PROJECT:
// 1. Delete the mock interfaces and functions below.
// 2. Uncomment the actual imports here to connect to your real data.
// ============================================================================
...
// --- MOCK DATA FOR CANVAS PREVIEW COMPILATION ---
...
export default function PrintableReport({ responses, questionsBank, finalResults, lang }: PrintableReportProps) {
  ...
  // Derive domain-specific badges safely
  ...
  return (
    <div 
      // overflow: hidden is removed to allow printing across multiple pages
      className="bg-white text-black p-12 w-[1200px] flex flex-col font-sans relative print:overflow-visible"
    >
      {/* HEADER LOGO & TITLE */}
      <div 
        className="flex justify-between items-center border-b-4 border-black pb-8 mb-10 w-full break-inside-avoid"
      ...
      >
        ...
        <div className="flex flex-col items-end">
          <div className="text-8xl font-black">{overallScore || 0}</div>
      ...
      <div className="flex flex-col lg:flex-row gap-12 w-full">
        ...
        <div className="flex-[3] flex flex-col gap-10">
          ...
          {/* Radar Chart Block */}
          <div 
            className="border border-gray-300 p-8 pt-4 rounded-[32px] bg-gray-50 break-inside-avoid"
            style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
          >
          ...
          <div className="flex flex-col gap-6 w-full">
            ...
            {critical && critical.length > 0 && (
              <div 
                className="border border-black bg-black text-white p-8 rounded-2xl flex flex-col shadow-lg break-inside-avoid"
                style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
              >
            ...
            {suggested && suggested.length > 0 && (
              <div 
                className="border-2 border-gray-300 bg-white text-black p-8 rounded-2xl flex flex-col break-inside-avoid"
                style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
              >
            ...
            {(!critical || critical.length === 0) && (!suggested || suggested.length === 0) && (
               <div 
                 className="p-8 border-2 border-gray-300 rounded-2xl text-center break-inside-avoid"
                 style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
               >
        ...
        <div className="flex-[2] flex flex-col gap-6">
           <h3 
             className="text-xl font-bold uppercase tracking-widest border-b-2 border-black pb-2 break-inside-avoid"
             style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
           >
           ...
           <div className="flex flex-col gap-6">
             {domains.map((d, i) => {
               ...
               return (
                 <div 
                   key={i} 
                   className="w-full flex items-center justify-between border border-gray-300 rounded-full p-2 pr-6 shadow-sm bg-gray-50 break-inside-avoid"
                   style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
                 >
             ...
             {domains.every(d => d.tier === 'None') && (
               <div 
                 className="p-8 text-center text-gray-400 font-bold uppercase break-inside-avoid"
                 style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
               >
      ...
      <div 
        className="w-full border-t border-gray-300 mt-12 pt-6 flex justify-between text-gray-400 text-xs font-bold uppercase tracking-widest break-inside-avoid"
        style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
      >
